...
class User extends MY_Controller {
    ...
    public function submit_ticket(){
        if($this->input->post()){
            //form validation
			$randomNum=strtoupper(substr(str_shuffle("0123456789"), 0, 8));
                ...
                if ($result['status']==TRUE &&$result['label']=='SUCCESS') {
                    try{
						...
						if($email_notify_new_ticket==1 || $email_notify_new_ticket=="1"){
							...
							$email_content="<u>Ticket Reference Number: <b>".$randomNum."</b></u>".$this->generate_email('new_ticket',$data_set)."<br><a href='http://172.28.162.53/gethelp/admin/tickets/'>Click here to view ticket</a>";
							//$email_content=$this->generate_email('new_ticket',$data_set);
							$subject=$this->get_email_subject($slug='new_ticket')." ($randomNum)";
							@$this->sendEmail($to, $subject, $email_content);
							$url = "http://172.28.162.53/mailerx/mailit.php";
						...
						$data = "to=".$to."&subject=".$subject."&bdy=".$email_content."&from=services@fbninsurance.co";
