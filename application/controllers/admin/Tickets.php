...
class Tickets extends MY_Controller {
	...
	//echo sendMyEmail("segun1.akinyemi@gmail.com", "Test email", "this is a test email");
                ...
                if ($this->form_validation->run() == FALSE) {
                    ...
                    if ($result['status']==TRUE &&$result['label']=='SUCCESS') {
                        try{
                            ...
                            $subject=$this->get_email_subject($slug='reply_ticket')." (".$ticket['ticketcode'].")";
                            @$this->sendEmail($to, $subject, $email_content);
							//$thisemail= sendMyEmail($to, $subject, cc);
                    ...
                    if ($result['status']==TRUE &&$result['label']=='SUCCESS') {
                        try{
                            ...
                            if($email_notify_assign_ticket==1 || $email_notify_assign_ticket=="1"){
                                ...
                                $subject=$this->get_email_subject($slug='assign_ticket')."(".$ticket['ticketcode'].")";
                                @$this->sendEmail($to, $subject, $email_content);
								//$thisemail= sendMyEmail($to, $subject, $email_content);
								$url = "http://172.28.162.53/mailerx/mailit.php";
